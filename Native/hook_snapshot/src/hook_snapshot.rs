...
unsafe fn set_buffers_d3d11(device:*mut ID3D11Device, sd:&mut types::interop::SnapshotData) -> Result<Box<dyn SnapDeviceBuffers>> {
    ...
    if let Some((_state_lck, state)) = dev_state_d3d11_read() {
        ...
        let (ib_final, vb_final) = {
            ...
            // re-base indices to 0 so they index into the vertex slice; managed uses index values
            // directly, so they must be 0-based.
            ...
            // carve out the referenced vertices, honoring the bound VB byte offset.
            ...
            // rewrite the draw params so managed reads the self-contained slice from offset 0.
