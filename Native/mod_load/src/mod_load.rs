...
pub unsafe fn setup_mod_data(device: DevicePointer, callbacks: interop::ManagedCallbacks) {
    ...
    for midx in 0..mod_count {
        ...
        fn insert_after<T, F>(vec: &mut Vec<T>, item: T, pred: F)
        where
            F: Fn(&T) -> bool,
        {
            let idx = vec.iter()
        ...
        //insert_after(vec, native_mod_data, |nmd| nmd.parent_mod_names.is_empty());
...
//                 nmod.name, nmod.mod_data.numbers.prim_count, nmod.mod_data.numbers.vert_count));
    ...
    for nmodv in loaded_mods.values_mut() {
        // sort mods so that variants (mods with no parents) are up front, this makes cycling easier
        nmodv.sort_by_key(|nmod| {
            // Key is a tuple:
            // (0: mods with no parents, 1: mods with parents, 2: deletion mods)
            // This will order: [variants], [normal], [deletions], preserving stable order.
...
pub unsafe fn load_deferred_mods(device: DevicePointer, callbacks: interop::ManagedCallbacks) {
        ...
        for nmd in to_load.iter() {
            ...
            if let Some(ref mut nmod) = nmod {
                ...
                if !nmod.mod_data.data_available {
                    ...
                    if mdat != null_mut() {
                        ...
                        if lres == 1 || (*mdat).data_available {
                            write_log_file(&format!("load_deferred_mods: XXupdating mod numbers for mod: {}", nmod.name));
                ...
                //write_log_file(&format!("checking for multithreaded device..."));

                let can_load_in_thread = (*DEVICE_STATE).multithreaded();
                //let can_load_in_thread = false;

                //write_log_file(&format!("multithreaded device: {}", can_load_in_thread));
                ...
                match device {
                    ...
                    DevicePointer::D3D11(dev) => {
                        write_log_file(&format!("beginning d3d 11 load: can load in thread: {}", can_load_in_thread));
