...
use winapi::um::dinput::{IID_IDirectInput8W,GUID_SysKeyboard,GUID_Key};
use winapi::ctypes::c_void;

//use winapi::shared::winerror::{S_OK};
use winapi::shared::guiddef::{REFIID, REFGUID, GUID};
// use winapi::ctypes::c_void;
...
use util;
use util::{Result, HookError, write_log_file};
...
interface IDirectInputDevice8W(IDirectInputDevice8WVtbl): IUnknown(IUnknownVtbl) {
    ...
    fn GetDeviceData(cbObjectData:DWORD,rgdod:LPVOID /*LPDIDEVICEOBJECTDATA*/,pdwInOut:LPDWORD,dwFlags:DWORD,) -> HRESULT,
...
interface IDirectInput8W(IDirectInput8WVtbl): IUnknown(IUnknownVtbl) {
    fn CreateDevice(dev:REFGUID, outdev:*mut *mut IDirectInputDevice8W, something: LPUNKNOWN,) -> HRESULT,
...
type DirectInput8CreateFn = unsafe extern "system" fn(hinst:HINSTANCE,
    dwVersion: DWORD,
    riidltf: REFIID,
    ppvOut: *mut LPVOID,
...
impl Input {
    ...
    unsafe fn init(&mut self) -> Result<()> {
        ...
        let hr = (create)(handle, DIRECTINPUT_VERSION, &IID_IDirectInput8W, &mut dinput8, null_mut() );
        if hr != 0 {
            return Err(HookError::DInputCreateFailed(format!("failed to create dinput8: {:x}", hr)));
        }
        ...
        if hr != 0 {
            return Err(HookError::DInputCreateFailed(format!("failed to create dinput keyboard: {:x}", hr)));
        }
        ...
        if hr != 0 {
            return Err(HookError::DInputCreateFailed(format!("failed to set data format on keyboard: {:x}", hr)));
        }
        let hr = (*keyboard).Acquire();
        if hr != 0 {
            return Err(HookError::DInputCreateFailed(format!("failed to acquire keyboard: {:x}", hr)));
        }
...
macro_rules! keyformat {
    ($x:expr) => {
        DIOBJECTDATAFORMAT(&GUID_Key, $x, DIDFT_OPTIONAL | DIDFT_BUTTON | (($x as u16) << 8) as DWORD, 0)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_input() {
        println!("sizeof DIOBJECTDATAFORMAT: {}", std::mem::size_of::<DIOBJECTDATAFORMAT>());
        println!("sizeof DIDATAFORMAT: {}", std::mem::size_of::<DIDATAFORMAT>());
        println!("c_dfDIKeyboard: {:?}", c_dfDIKeyboard);
