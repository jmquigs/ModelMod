...
def create_mesh(new_objects,
                ...
                vertex_groups,
                # ModelMod requires the next three extra input arguments
                weighted_groups,
                pos_xforms,
                uv_xforms,
                dataname,
    ...
    # ModelMod requires addition of weighted vertex groups for skinning
    ...
    # Fros comment: Transform strings are now attached to object via custom properties
    for xform_index, xform in enumerate(pos_xforms):
        xform = xform.decode('utf-8', "replace")
        # xform = "PosTransform." + xform
        # group = ob.vertex_groups.new(name=xform)
        ob.data['ModelMod_Pos_{}'.format(xform_index)] = xform

    for xform_index, xform in enumerate(uv_xforms):
        xform = xform.decode('utf-8', "replace")
        # xform = "UVTransform." + xform
        # group = ob.vertex_groups.new(name=xform)
        ob.data['ModelMod_UV_{}'.format(xform_index)] = xform
