...
def write_file(filepath, objects, depsgraph, scene,
    ...
    with ProgressReportSubstep(progress, 2, "OBJ Export path: %r" % filepath, "OBJ Export Finished") as subprogress1:
        with open(filepath, "w", encoding="utf8", newline="\n") as f:
            ...
            # ModelMod mappings of vertex groups to indices; shared between all objects
            indexedGroupDict = {}
            indexedGroupList = []
            ...
            for i, ob_main in enumerate(objects):
                ...
                for ob, ob_mat in obs:
                    with ProgressReportSubstep(subprogress1, 6) as subprogress2:
                        ...
                        # ModelMod write vertex groups
                        vertGroupNames = ob.vertex_groups.keys()
                        blendGroupPrefix = "Index."
                        
                        if vertGroupNames:
                            # Fros comment: All vertex groups besides those with the Index prefix are ignored. Useful if arbtirary extra groups have been added
                            # pos_xforms = []
                            # uv_xforms = []
            ...
            customPropKeys = ob.data.keys()
            
            if posTransformPrefix + '0' in customPropKeys:
                fw('#pos_xforms ')
                for key in customPropKeys:
                    if key.startswith(posTransformPrefix):
                        fw(ob.data[key] + ' ')
                fw('\n')
                
            if uvTransformPrefix + '0' in customPropKeys:
                fw('#uv_xforms ')
                for key in customPropKeys:
